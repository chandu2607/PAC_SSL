...
from lopo_evaluation import PATIENTS_ALL
...
from run_ultimate_hybrid_fusion_70 import train_balanced_ridge

CACHE_V2 = Path("data/preprocessed/encoder_features_z_v2.h5")
CACHE_WNN = Path("data/preprocessed/wnn_features_cache.pt")
SAVE_PATH = Path("data/final_patient_classifiers.pt")

def main():
    if hasattr(sys.stdout, "reconfigure"):
        sys.stdout.reconfigure(encoding="utf-8", errors="replace")
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"=== TRAINING AND SAVING 17 PATIENT-SPECIFIC CLASSIFICATION HEADS ===", flush=True)
    
    if not CACHE_WNN.exists():
        print(f"Cache file {CACHE_WNN} not found! Cannot train classifiers.", flush=True)
        return
    ...
    for p in sorted(patient_data.keys()):
        ...
        classifiers_dict[p] = head.state_dict()
        print(f"Trained and captured classifier head for patient: {p}", flush=True)
        
    print(f"\nSaving classifier heads dict to {SAVE_PATH}...", flush=True)
    torch.save(classifiers_dict, SAVE_PATH)
    print("Classifiers saved successfully!", flush=True)
