"""
run_epoch1_surrogates.py — 20-Shuffle Surrogate Test on Epoch 1 Checkpoint
Tests chb19 and chb03 specifically on Epoch 1 representations (`encoder_features_z_epoch1.h5`)
to confirm whether the apparent raw AUC improvement (chb19: 0.8509, chb03: 0.7436) clears the <5% surrogate bar.
Also runs chb20 and chb02 for complete visibility.
"""
import time
import h5py
import numpy as np
...
from run_prestage4_item_b import EPOCH1_CACHE_H5, load_features_dict

def main():
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"=== Epoch 1 Checkpoint 20-Shuffle Surrogate Test starting on {device} ===", flush=True)
    
    if not EPOCH1_CACHE_H5.exists():
        raise FileNotFoundError(f"Cache {EPOCH1_CACHE_H5} not found!")
        
    features_dict = load_features_dict(EPOCH1_CACHE_H5)
    print(f"Loaded {len(features_dict)} subjects from Epoch 1 cache.", flush=True)
    
    subjects_to_test = ["chb19", "chb03", "chb20", "chb02"]
    n_shuffles = 20
    
    for test_p in subjects_to_test:
        ...
        print(f"\n==========================================", flush=True)
        print(f"[{test_p} - EPOCH 1] Real LOPO AUC: {real_auc:.4f}", flush=True)
        print(f"Running {n_shuffles} surrogate shuffles across Epoch 1 training fold...", flush=True)
        
        surrogate_aucs = []
        for s in range(1, n_shuffles + 1):
            torch.manual_seed(3000 + int(test_p[3:]) * 100 + s)
        ...
        print(f"\n--- [{test_p} - EPOCH 1] Surrogate Distribution Summary ---", flush=True)
